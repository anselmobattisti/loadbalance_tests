...
class iplb (object):
  ...
  def __init__ (self, connection, service_ip, servers = []):
    ...
    self.sorted_live_servers = {} # IP -> MAC,port
  ...
  def _pick_server_round_robin_weight (self, key, inport):  
    ...
    """
    global server_order, server_num

    server_num = server_num + 1 

    if server_num >= len(server_order):
      server_num = 0

    ss = server_order[server_num]

    print(ss)
    
    return self.live_servers.keys()[ss]
    
  def _handle_PacketIn (self, event):
    global strategy_choosed
...
def launch (ip, servers, strategy = RANDON, dpid = None):
  global _dpid, strategy_choosed, server_order
  if dpid is not None:
